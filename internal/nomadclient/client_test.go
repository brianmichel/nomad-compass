...
func TestDeriveStatus(t *testing.T) {
	cases := []struct {
		...
		{name: "healthy despite historical failures", status: &JobStatus{RunningAllocs: 1, DesiredAllocs: 1, FailedAllocs: 3}, want: "healthy"},
		{name: "degraded", status: &JobStatus{RunningAllocs: 1, DesiredAllocs: 2}, want: "degraded"},
		{name: "degraded with failed allocs", status: &JobStatus{RunningAllocs: 1, DesiredAllocs: 2, FailedAllocs: 1}, want: "degraded"},
